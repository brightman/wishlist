...
typedef enum{
    BD_SOCIAL_SHARE_WX_CONTENT_TYPE_WEBPAGE,    //微信分享内容为网页
    BD_SOCIAL_SHARE_WX_CONTENT_TYPE_IMAGE       //微信分享内容为图片
} BD_SOCIAL_SHARE_WX_CONTENT_TYPE;
...
/**
 *  @brief	设置微信分享内容的类型
 *	@param 	type         微信分享内容的类型
 *
 *  微信默认的分享内容的类型是网页类型，分享后图片会作为内容的缩略图进行展示。
 *  当分享内容的类型设置为图片类型，分享后图片作为分享内容，可以在微信内展示大图。
 *  如果没有添加图片资源，此方法设置无效，使用默认的网页类型进行分享。
 */
-(void)setWXShareContentType:(BD_SOCIAL_SHARE_WX_CONTENT_TYPE)type;
